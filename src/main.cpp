...
void player_movement()
 {
  ...
  // clear_keybuf();
   ...
   player.gun = FAST_GUN;
   for(long int a = 0; a < (gun[selected_gun].ammo + pl_bullet_number); a++)
    {
    if(bullet[a].exists == DEAD)
    {
    bullet[a].speed = gun[selected_gun].speed;
...
void set_gun_values()
 {  
  ...
  gun[NORMAL_GUN].ammo = 100;
  gun[FAST_GUN].speed = 4;
  gun[FAST_GUN].power = 5;
  gun[FAST_GUN].ammo = 300;
 } 
 ...
 y[4] = y[0] + 40; //* -cos(player.angle);
  ...
  if(e_move == true)
   ...
   { 
    ...
    if((ey[number][num] <= ey[e][num] + enemy[e].height + 1) 
           && (ey[number][num] + enemy[number].height >= y[num] - 1)
           && (ex[number][num] <= ex[e][num] + enemy[e].width + 1)
           && (ex[number][num] + enemy[number].width >= ex[e][num] - 1))
       {
       e_move = false;
       enemy[number].exists = ENEMY_DIE; 
       break;
...
void kill_bullet()
 {
  for(int a = 0; a < pl_bullet_number; a++)
  {
   ...
   {
   for(int b = 1; b < pl_bullet_number; b++)
   {
